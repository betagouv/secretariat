  ...
  return (
    <>
      ...
      <p>
        ...
        <br />
        Pour demander un badge, il faut que tu viennes au minimum deux fois par
        semaine.
      </p>
      <p>
        Sinon tu devras demander un badge invité à chacune des tes venues. Ce
        sont les régles du batiment Ségur nous n'y pouvont rien.
      </p>
      ...
      <button onClick={askForBadge} className="button no-margin">
        Faire la demande de badge
      </button>
    </>
  );
};
...
export const Badge = InnerPageLayout(function (props: Props) {
    ...
    stepView = (
      <>
        ...
        {dsToken && !props.dossier && (
          <>
            <p>
              Votre démarche a été préremplie, vous pouvez maintenant cliquer
              sur le bouton ci-dessous pour la terminer.
            </p>
            <p>⚠️ Veillez à respecter les règles suivantes :</p>
            <ul>
              <li>
                Le compte à utiliser pour remplir la demande doit être lié à
                votre adresse : {props.primaryEmail}
              </li>
              <li>
                La date de fin de votre badge a été préremplie, si vous la
                changez, mettez maximum 1 an{' '}
              </li>
            </ul>
            <a
              className="button"
  ...
  return (
    <div className="container">
      {props.dossier && props.dossier.state === 'accepte' && (
        <div className="panel margin-top-m" style={{ minHeight: 500 }}>
          <h2>Badge</h2>
          <p>
            <b>Date de fin de validité du badge :</b>{' '}
            {
              props.dossier.champs.find(
                (c) => c.label === 'Date de fin de mission'
              ).stringValue
            }
          </p>
        </div>
