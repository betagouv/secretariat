...
function getCurrentPhaseDate(startup: StartupInfo): Date | null {
  ...
  if (startup.attributes.phases) {
    date =
      startup.attributes.phases[startup.attributes.phases.length - 1].start ||
      undefined;
    if (date) {
      date = new Date(date);
    }
...
export async function syncBetagouvStartupAPI() {
  ...
  for (const startup of startups) {
    ...
    for (const member of startupDetailInfo.active_members) {
      if (
        members.find(
          (m) => m.id === member && m.domaine === Domaine.INTRAPRENARIAT
        )
      ) {
        has_intra = true;
      }
      if (
        members.find((m) => m.id === member && m.domaine === Domaine.COACHING)
      ) {
        has_coach = true;
      }
    }
