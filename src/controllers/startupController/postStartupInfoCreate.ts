...
export async function postStartupInfoCreate(req, res) {
  try {
    ...
    const startup =
      req.body.startup || requiredError('nom du produit', errorHandler);
    ...
    const analyse_risques =
      req.body.analyse_risques === 'true' || req.body.analyse_risques === true
        ? true
        : false;
    ...
    const content =
      req.body.text || requiredError('description du produit', errorHandler);
