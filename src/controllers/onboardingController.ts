...
async function sendMessageToReferent({ prInfo, referent, username, isEmailBetaAsked, name }: IMessageInfo) {
  ...
  const messageContent = await ejs.renderFile('./views/templates/emails/onboardingReferent.ejs', {
...
export async function postForm(req, res) {
  ...
  try {
    ...
    const content = await ejs.renderFile('./views/templates/markdown/githubAuthor.ejs', {
      name,
      description,
      website,
      github,
