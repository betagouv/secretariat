...
async function sendOnboarderRequestEmail(newcomer: Member, onboarder: Member) {
  ...
  const html = await ejs.renderFile('./views/templates/emails/marrainageRequest.ejs', {
    newcomer,
    onboarder,
    marrainageAcceptUrl,
...
export async function acceptRequest(req, res) {
  try {
    ...
    const html = await ejs.renderFile('./views/templates/emails/marrainageAccept.ejs', {
      newcomer,
      onboarder,
    });
