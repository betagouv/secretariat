...
async function createNewcomerGithubFile(username, content, referent) {
  ...
  const prInfo = await getGithubMasterSha()
    ...
    .then(() => {
      console.log(
        `Fiche Github pour ${username} créée dans la branche ${branch}`
      );
      return makeGithubPullRequest(
        branch,
        `Création de fiche pour ${username}. Référent : ${
          referent || 'pas renseigné'
        }.`
      );
    })
    .then((response) => {
    ...
    .catch((err) => {
      ...
      if (err.status === 422) {
        ...
        throw new Error(
          `Erreur Github lors de la création de la fiche de ${username}`
        );
      }
    });
  return prInfo;
...
export async function postOnboardingFormApi(req, res) {
  postOnboardingData(
    req,
    res,
    ({ prInfo, isEmailBetaAsked }) => {
      res.json({
        prInfoNumber: prInfo.number,
        isEmailBetaAsked,
      });
    },
    (data) => {
...
async function postOnboardingData(req, res, onSuccess, onError) {
  ...
  try {
    const firstName =
      req.body.firstName || requiredError('prénom', errorHandler);
    const lastName =
      req.body.lastName || requiredError('nom de famille', errorHandler);
    const description = req.body.description || null;
    const role = req.body.role || requiredError('role', errorHandler);
    const start =
      req.body.start || requiredError('début de la mission', errorHandler);
    const end =
      req.body.end || requiredError('fin de la mission', errorHandler);
    ...
    const referent =
      req.body.referent || requiredError('référent', errorHandler);
    const domaine = isValidDomain('domaine', req.body.domaine, errorHandler)
      ? req.body.domaine
      : null;
    const inputEmail = isValidEmail(
      'email pro/perso',
      req.body.email,
      errorHandler
    )
      ? req.body.email
      : null;
    const should_create_marrainage =
      req.body.should_create_marrainage === 'true' || false;
    ...
    const memberType =
      req.body.memberType || requiredError('memberType', errorHandler);
    ...
    if (
      ...
      errorHandler(
        'legal_status',
        `Le statut legal n'a pas une valeur autorisée`
      );
    }
    if (
      gender &&
      !genderOptions.map((genderOption) => genderOption.key).includes(gender)
    ) {
    ...
    if (
      ...
      errorHandler(
        'workplace_insee_code',
        `Le lieu de travail principal n'as pas été trouvé`
      );
    }
    ...
    if (
      memberType &&
      !config.user.memberOptions.map((opt) => opt.key).includes(memberType)
    ) {
      errorHandler(
        'memberType',
        `Le type de membre n'est pas une valeur autorisée.`
      );
    }
    const website = isValidUrl(
      'Site personnel',
      req.body.website,
      errorHandler
    );
    const github = shouldBeOnlyUsername(
      'Utilisateur Github',
      req.body.github,
      errorHandler
    );
    ...
    if (startDate && endDate) {
      if (startDate < new Date(config.user.minStartDate)) {
        formValidationErrors[
          'date de début'
        ] = `La date doit être au moins ${config.user.minStartDate}`;
      }
      if (endDate < startDate) {
        formValidationErrors['date de fin'] =
    ...
    const hasChosenSecondaryEmail =
      req.body.communication_email === CommunicationEmailCode.SECONDARY &&
      secondary_email;
    const communication_email = hasChosenSecondaryEmail
