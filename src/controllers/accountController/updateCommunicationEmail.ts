...
export async function updateCommunicationEmailApi(req, res) {
  updateCommunicationEmailHandler(
    req,
    res,
    () => {
      res.json({
...
export async function updateCommunicationEmailHandler(
  req,
  res,
  onSuccess,
  onError
  ...
  try {
    ...
    if (communication_email != previousCommunicationEmail && hasBothEmailsSet) {
      ...
      req.flash(
        'message',
        "Ton choix d'email de communication a bien été mis à jour."
      );
      console.log(
        `${req.auth.id} a mis à jour son choix d'email de communication.`
      );
    }
    onSuccess();
  } catch (err) {
