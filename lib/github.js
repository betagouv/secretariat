...
async function getAuthorFileList(hash) {
  ...
  const contentObject = beforeRootTree.tree.find(function(element) {
    return element.path === "content"
  })

  const beforeContentTree = await getJson(contentObject.url)
  const authorsObject = beforeContentTree.tree.find(function(element) {
    return element.path === "_authors"
  })
...
async function fetchDetails(input) {
  ...
  return await Promise.all(changes.map(async function(data) {
